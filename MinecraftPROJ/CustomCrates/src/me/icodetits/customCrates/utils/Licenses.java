...
public class Licenses {
	...
	public Licenses(String licenseKey, String validationServer, Plugin plugin) {
		this.licenseKey = licenseKey;
		this.plugin = plugin;
		this.validationServer = validationServer + (validationServer.endsWith("/") ? "verify.php" : "/verify.php");
	}

	public Licenses setSecurityKey(String securityKey) {
	...
	public boolean register() {
		...
		if (vt.equals(ValidationType.URL_ERROR)) {
			log(0, "[ERROR] Couldn't connect to License-Server!");
		} else {
			log(0, "[CONNECTED] Successfully connected to License-Server!");
		}

		if (vt == ValidationType.VALID) {
			log(1, "[SUCCESS] License valid!");
			log(0, "[]==========[License-System]==========[]");
			return true;
		} else {
			log(1, "[FAILED] License is NOT valid!");
			log(1, "  > Failed as a result of " + vt.toString());
			log(0, "[RESULT] Disabling plugin!");
			log(0, "[]==========[License-System]==========[]");
